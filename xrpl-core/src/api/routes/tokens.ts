...
import { 
  ...
  authorizeMPToken, 
  destroyMPToken 
} from '../../services/mpt/MPTokenManager';
...
router.post('/mpt/create', async (req, res) => {
  try {
    ...
    const result = await createMPToken(
      adminSeed,
      userSeed,
      assetScale || 0,
...
router.post('/mpt/optin', async (req, res) => {
  try {
    ...
    const result = await optInMPToken(userSeed, issuanceId);
...
router.post('/mpt/send', async (req, res) => {
  try {
    ...
    const result = await sendMPToken(
      adminSeed,
      userSeed,
      issuanceId,
      destinationAddress,
...
router.post('/mpt/authorize', async (req, res) => {
  try {
    ...
    const result = await authorizeMPToken(
      adminSeed,
      issuanceId,
      holderAddress,
      isUnauthorize || false
...
router.delete('/mpt/:issuanceId', async (req, res) => {
  try {
    ...
    const result = await destroyMPToken(adminSeed, issuanceId);
