...
async function testSimpleFlow() {
  ...
  try {
    ...
    if (projectResponse.data.success) {
      ...
      const getProjectResponse = await axios.get(`${API_BASE}/projects/${project.projectId}`);
      ...
      try {
        ...
        console.log('❌ Investment failed:', (error as any).response?.data?.message || error.message);
      }
      ...
      try {
        const milestoneResponse = await axios.post(
          `${API_BASE}/projects/${project.projectId}/milestones/milestone_${project.projectId}_1/achieve`,
          {
        ...
        console.log('❌ Milestone achievement failed:', (error as any).response?.data?.message || error.message);
      }
      ...
      try {
        ...
        console.log('❌ Project list failed:', (error as any).response?.data?.message || error.message);
      }
