  ...
  try {
    ...
    try {
      ...
      if (projectsResponse.data.success) {
        ...
        const foundProject = projects.find((p: any) => p.id === project.projectId);
        if (foundProject) {
          console.log('✅ Project found in list:', foundProject.name);
        } else {
          console.log('❌ Project NOT found in list');
          console.log('Available projects:', projects.map((p: any) => p.id));
        }
      } else {
    ...
    try {
      const projectResponse2 = await axios.get(`${API_BASE}/projects/${project.projectId}`);
