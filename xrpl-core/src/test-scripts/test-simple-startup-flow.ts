...
async function testSimpleStartupFlow() {
  ...
  try {
    ...
    const projectResponse = await axios.post('http://localhost:3001/api/projects', simpleProjectData);
    ...
    if (projectResponse.status === 201) {
      ...
      try {
        const milestoneResponse = await axios.post(`http://localhost:3001/api/projects/${projectId}/milestones/milestone_1/achieve`, {
