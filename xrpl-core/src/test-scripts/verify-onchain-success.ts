    ...
    for (let i = 0; i < 30; i++) {
      ...
      try {
        ...
        if (ledgerData.result.ledger.transactions) {
          // 우리 지갑과 관련된 트랜잭션 필터링
          const relevantTransactions = ledgerData.result.ledger.transactions.filter((tx: any) => 
            testWallets.includes(tx.Account) || 
          ...
          if (relevantTransactions.length > 0) {
            ...
            relevantTransactions.forEach((tx: any, index: number) => {
              console.log(`\n   ${index + 1}. Transaction:`);
