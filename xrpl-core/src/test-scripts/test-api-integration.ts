...
async function testAPIIntegration() {
  ...
  try {
    ...
    const projectResponse = await axios.post(`${API_BASE_URL}/api/projects`, projectData);
    ...
    if (projectResponse.data.success) {
      ...
      const getProjectResponse = await axios.get(`${API_BASE_URL}/api/projects/${projectId}`);
      ...
      const investmentResponse = await axios.post(`${API_BASE_URL}/api/investments`, investmentData);
      ...
      const milestoneResponse = await axios.post(
        `${API_BASE_URL}/api/projects/${projectId}/milestones/${milestoneId}/achieve`,
        milestoneData
      );
    ...
    console.error('❌ API test failed:', (error as any).response?.data || (error as Error).message);
  }
}
