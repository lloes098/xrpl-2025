...
async function testNewStartupFlow(): Promise<void> {
  ...
  try {
    ...
    for (const investor of investors) {
      try {
        ...
        const balanceResponse = await axios.get(`${API_BASE}/tokens/balance/${wallet.address}`);
    ...
    const revenueResponse = await axios.post(
      `${API_BASE}/projects/${projectResponse.data.data.projectId}/revenue`,
      revenueData
    );
