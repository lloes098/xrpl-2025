...
async function testSimpleAPI() {
  ...
  try {
    ...
    if (projectResponse.data.success) {
      ...
      const getProjectResponse = await axios.get(`${API_BASE_URL}/api/projects/${projectResponse.data.data.projectId}`);
    ...
    console.error('❌ Simple API test failed:', (error as any).response?.data || (error as Error).message);
  } finally {
