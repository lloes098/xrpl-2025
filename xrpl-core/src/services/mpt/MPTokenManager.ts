...
export class MPTokenManager {
  ...
  async createIssuance(
    ...
    // 플래그 계산 (XRPL 공식 플래그 값 사용)
    let txFlags = 0
    if (flags.tfMPTCanTransfer) txFlags |= 0x00000020  // lsfMPTCanTransfer
    if (flags.tfMPTCanEscrow) txFlags |= 0x00000008    // lsfMPTCanEscrow
    if (flags.tfMPTRequireAuth) txFlags |= 0x00000004  // lsfMPTRequireAuth
    if (flags.tfMPTCanTrade) txFlags |= 0x00000010     // lsfMPTCanTrade
    if (flags.tfMPTCanLock) txFlags |= 0x00000002      // lsfMPTCanLock
    ...
    try {
      console.log("MPT 생성 트랜잌션:", JSON.stringify(tx, null, 2))
      
      // 트랜잌션 준비 및 서명
      const prepared = await this.client.autofill(tx)
      console.log("준비된 트랜잌션:", JSON.stringify(prepared, null, 2))
      
      const signed = this.adminWallet.sign(prepared)
      console.log("서명된 트랜잌션:", signed.tx_blob)
      ...
      const transactionResult = (result.result as any)?.meta?.TransactionResult
      ...
      if (engineResult !== 'tesSUCCESS' && transactionResult !== 'tesSUCCESS') {
        ...
        throw new Error(`MPT 생성 실패: ${errorMessage}`)
      }
