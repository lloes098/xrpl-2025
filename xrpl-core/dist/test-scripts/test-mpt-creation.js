    ...
    try {
        ...
        if (mptResult.result.validated) {
            ...
            if (accountObjects.result.account_objects && accountObjects.result.account_objects.length > 0) {
                ...
                const recipientWallet = xrpl_1.Wallet.generate();
                ...
                const mptTransferTx = {
                    ...
                    Amount: {
                        mpt_issuance_id: mptObject.MPTokenIssuanceID,
                        value: '1000' // 1000 토큰 전송
                    },
                    Fee: '12'
                };
                const transferResult = await client.submitAndWait(mptTransferTx, { wallet: testWallet });
                if (transferResult.result.validated) {
                    ...
                    if (recipientObjects.result.account_objects && recipientObjects.result.account_objects.length > 0) {
                        ...
                        console.log(`   - Recipient MPT Balance: ${recipientMpt.MPTAmount || '0'}`);
                    }
                    else {
            ...
            console.log(`   - Error: ${mptResult.result.meta?.TransactionResult}`);
        }
        // 7. 블록스캔 링크 생성
...
//# sourceMappingURL=test-mpt-creation.js.map