...
async function checkSuccessfulMPT() {
    ...
    try {
        ...
        try {
            ...
            console.log(`      - Type: ${txInfo.result.TransactionType || 'N/A'}`);
            console.log(`      - Ledger Index: ${txInfo.result.ledger_index}`);
            console.log(`      - Date: ${txInfo.result.date ? new Date(txInfo.result.date * 1000).toISOString() : 'N/A'}`);
            if (txInfo.result.TransactionType === 'MPTokenIssuanceCreate') {
                console.log(`      - MPT Issuance ID: ${txInfo.result.MPTokenIssuanceID || 'N/A'}`);
                console.log(`      - Maximum Amount: ${txInfo.result.MaximumAmount || 'N/A'}`);
                console.log(`      - Asset Scale: ${txInfo.result.AssetScale || 'N/A'}`);
                console.log(`      - Transfer Fee: ${txInfo.result.TransferFee || 'N/A'}`);
            }
...
//# sourceMappingURL=check-successful-mpt.js.map