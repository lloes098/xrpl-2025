...
async function testDevnetConnection() {
    ...
    try {
        ...
        try {
            ...
            // 수동으로 XRP 전송 시도
            const faucetWallet = xrpl_1.Wallet.fromSeed('sEdTJSpenzy1XgBAaMufgR6dH4Wj1Z'); // Devnet faucet
            const payment = {
            ...
            await client.submitAndWait(payment, { wallet: faucetWallet });
        ...
        const destinationWallet = xrpl_1.Wallet.generate();
        const paymentTx = {
            TransactionType: 'Payment',
            Account: testWallet.address,
        ...
        const paymentResult = await client.submitAndWait(paymentTx, { wallet: testWallet });
        ...
        console.log(`   - Status: ${txInfo.result.meta?.TransactionResult}`);
        console.log(`   - Fee: ${txInfo.result.Fee} drops`);
        console.log(`   - Sequence: ${txInfo.result.Sequence}`);
...
//# sourceMappingURL=test-devnet-connection.js.map