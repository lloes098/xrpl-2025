...
async function testEnhancedStartupFlow() {
    ...
    try {
        ...
        for (let i = 0; i < investors.length; i++) {
            ...
            try {
                ...
                console.log(`❌ ${investor.name} investment error: ${error.response?.data?.message || 'Unknown error'}`);
            }
        }
        // Step 3: Check project status
        console.log('\n📊 Step 3: Checking Project Status...');
        const projectStatusResponse = await axios_1.default.get(`${API_BASE}/projects/${projectId}`);
        ...
        for (let i = 0; i < investors.length; i++) {
            const investor = investors[i];
            try {
                const balanceResponse = await axios_1.default.get(`${API_BASE}/tokens/balance/${investor.seed}`);
                ...
                console.log(`❌ ${investor.name} balance check error: ${error.response?.data?.message || 'Unknown error'}`);
            }
        }
        ...
        try {
            const revenueResponse = await axios_1.default.post(`${API_BASE}/projects/${projectId}/revenue`, revenueData);
            ...
            console.log(`❌ Revenue distribution error: ${error.response?.data?.message || 'Unknown error'}`);
        }
        // Step 6: Final project status
        console.log('\n📈 Step 6: Final Project Status...');
        const finalStatusResponse = await axios_1.default.get(`${API_BASE}/projects/${projectId}`);
